...
public class CustomerController {
    ...
    public ResponseEntity<Page<CustomerDto.Response>> getCustomerList(@ApiParam(value = "검색할 키워드", required = true) String keyword,
    ...
    public ResponseEntity<List<CustomerDto.Response>> getCustomerListAsIn(@ApiParam(value = "조회할 고객 아이디 배열", required = true) Long[] customerIds) {
    ...
    public ResponseEntity<URI> postCustomer(@ApiParam(value = "신규 고객 생성 정보")
                                            @RequestBody
                                            @Valid CustomerDto.Post dto,
                                            Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<CustomerDto.Response> getCustomer(@ApiParam(value = "조회할 고객의 아이디", required = true)
    ...
    public ResponseEntity<CustomerDto.Response> putCustomer(@ApiParam(value = "수정할 고객의 아이디", required = true)
                                                            @PathVariable("id") Long id,
                                                            @ApiParam(value = "수정할 고객 정보", required = true)
                                                            @RequestBody
                                                            @Valid CustomerDto.Put customerDto,
                                                            Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.badRequest().build();
        }
    ...
    public ResponseEntity deleteCustomer(@ApiParam(value = "제거할 고객의 아이디", required = true)
                                            @PathVariable("id") Long id) {
    ...
    public ResponseEntity postCustomerTag(@ApiParam(value = "고객의 아이디", required = true)
                                            @PathVariable("id") Long customerId,
                                            @ApiParam(value = "추가할 태그 타이틀", required = true)
        ...
        return ResponseEntity.ok().body(customerService.addTag(optionalCustomer.get(), tag));
    }
    ...
    public ResponseEntity deleteCustomerTag(@ApiParam(value = "고객의 아이디", required = true)
                                            @PathVariable("id") Long customerId,
                                            @ApiParam(value = "제거할 태그 타이틀", required = true)
                                            @RequestBody String title) {
        Optional<Customer> optionalCustomer = customerService.findById(customerId);
        if (optionalCustomer.isEmpty()) {
            return ResponseEntity.badRequest().build();
        ...
        return ResponseEntity.ok().body(customerService.removeTag(optionalCustomer.get(), optionalTag.get()));
    }
    ...
    public ResponseEntity<List<CustomerTypeDto>> getCustomerType() {
